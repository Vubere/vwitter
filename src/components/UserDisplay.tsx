      ...
      <div className="flex gap-2 h-[40px] border-box overflow-hidden">
        ...
        <Link to={`/profile/${details.username}`}>
          <div className="h-[40px]">
            <p className="m-0 p-0 h-[20px] text-white break-words text-[12px]">{details.name.length < 22 ? details.name : details.name.slice(0, 22) + '...' }</p>
            <p className="m-0 p-0 h-[15px] text-[#fff6] text-[12px]">@{details.username.length < 22 ? details.username : details.username.slice(0, 22) + '...'}</p>
          </div>
        </Link>
      </div>
